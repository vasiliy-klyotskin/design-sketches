...
struct Widget {
    ...
    /// Данные виджета
    ///
    /// Данные представлены в виде **Swift.Data**. Это решает проблему динамической типизации.
    ///
    ...
    /// Виджет является контейнером, если у него айди позиционирования не равен предопределенному айди НЕ котейнера
    var isContainer: Bool {
        id.positioning != WidgetId.positioningIdForNotContainers
    }
    
    /// Возвращает **true** если виджет, у которого вызывается этот метод и ``other`` отличаются состоянием, имея при этом одинаковый инстанс.
    ...
    /// Возвращает **true** если виджет, у которого вызывается этот метод и ``other`` отличаются позиционированием, имея при этом одинаковый инстанс.
    /// - Parameter from: Виджет, с которым нужно провести сравнение.
